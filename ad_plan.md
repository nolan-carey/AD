...
| 2 | **Swipe-up + centered brand + tagline pill + sparkle orbits logo + 4-feature constellation + iPhone** | 0:06 – 0:10.4 | 180 – 312 | 4.4s | Vertical swipe-up clears → silence → **CENTERED** brand lockup (Kiva chevron + "Kiva." wordmark, both stay centered entire sequence) → glassmorphism tagline pill POPS in/out → AI sparkle emerges from logo → orbits clockwise dropping 4 features at clock positions (12/3/6/9), all persist on screen → constellation moment (logo + 4 satellite features) → particles vortex to center → logo cross-fades into iPhone → dashboard with "All your admin. One place." (v1.26) |
...
**Time:** 0:06.0 – 0:10.4 (frames 180–312 · 4.4s · 132 frames) · **v1.26: revised per user direction (2026-05-07) — logo stays CENTERED the whole sequence; tagline gets a pop-in pill; the 4 features now CIRCLE the centered logo (not scattered to quadrants).**
...
| 0:07.2–0:07.5 | 216–225 | **🫧 Tagline pill POPS in below the wordmark.** A glassmorphism pill (rounded rectangle, full pill radius — `border-radius: 999px`) materializes ~22 px below the "Kiva." wordmark with a confident POP entrance:<br>• **Pill chrome:** `<GlassPlate>` variant — `bg: rgba(255,255,255,0.10)`, `backdrop-filter: blur(24px) saturate(160%)`, `border: 1px solid rgba(255,255,255,0.18)`, soft shadow. Padding 10 px vertical / 22 px horizontal.<br>• **Pill content:** *"Blue collar solutions to blue collar problems"* — Inter_500Medium ~16 px, white at 95% opacity.<br>• **POP animation:** scale 0 → 1.12 (overshoot) → 1.0 over 9 frames using POP_IN preset (spring damping 12). The overshoot is the "pop" — pill briefly bigger than rest, then settles.<br>Audio: soft "pop" hit (Phase 3 — could be `impact2.mp3` at 25% pitched up, or a generated bubble pop). |
| 0:07.5–0:08.0 | 225–240 | **Pill holds — viewer reads.** Static glass pill. Gentle inner shimmer (subtle highlight sweep across the glass, left → right, 0.5s loop). Logo + Kiva. behind/above remain centered. **15-frame hold lets the tagline land emotionally.** Audio: ambient drone underbed only. |
...
| 0:08.4–0:09.5 | 252–284 | **🌀 4-feature flash — sparkle CIRCLES the centered logo dropping each feature at a clock position. ALL FEATURES PERSIST on screen as the sparkle continues orbiting (per user direction 2026-05-07).** The sparkle traces a clockwise orbit at ~280 px radius from the logo center. At each clock position it PULSES and a feature TEXT BURSTS at that position — and the text **STAYS** there for the rest of the feature reveal. By F284 all 4 features are visible simultaneously, arrayed around the central logo. The orbital path is visible as a faint purple trail. ~8 frames per feature.<br><br>**Per-feature pattern (8f each):**<br>F0–F2: sparkle traces orbit arc toward the next clock position (motion blur trail follows the orbit)<br>F2: sparkle PULSES BRIGHT at landing position (scale 1.0 → 1.4 → 1.0) — burst of ~12 purple particles outward<br>F2–F5: feature text appears at that clock position with kinetic typography (see below)<br>F5–F8: text **HOLDS** (does NOT dissolve) while sparkle continues to next clock position<br><br>**Kinetic typography rule (UNCHANGED from v1.24 — user said "I like the current kinetic circling text"):** verb small (~32 px Inter_400Regular at 80% opacity), **outcome word HUGE** (~64 px Inter_700Bold at 100% white) with brief scale punch (1.0 → 1.08 → 1.0 over 3 frames). Period animates separately with a 1-frame delay. Soft purple glow underline beneath the bold word.<br><br>**Clock-position layout — features circle the centered logo at ~280 px radius. Each feature is a STACKED COMPOSITION: icon on top, text below.**<br><br>**Per-feature visual stack (vertical, anchored at the clock position):**<br>1. **Icon** (~48 px, line-style, white stroke ~3 px with subtle purple glow underneath, animated per the feature's meaning — see below)<br>2. *Verb* (small, Inter_400Regular ~32 px white at 80%)<br>3. ***Outcome word.*** (HUGE, Inter_700Bold ~64 px white, scale punch on entry, soft purple underline ~3 px below)<br><br>**🎙 Feature 1 — F252–F260: 12 o'clock (above logo, ~(960, 220))** — *"Speak"* + ***"quotes."***<br>**Icon: microphone → recording animation.** A line-style microphone icon appears (~48 px). Then concentric sound-wave arcs emanate outward from the mic in a rhythmic pulse, like real voice recording. F0–F2: mic line-draws in (stroke draws on). F2–F4: 3 expanding waveform arcs (each fades in then out as the next launches). F4–F8: continuous gentle pulse — subtle 1.0 → 1.05 scale on the mic body + waveform arcs loop softly. **Once placed, the mic continues this gentle pulsing while the feature persists** — the whole orbital constellation feels "alive."<br><br>**👤 Feature 2 — F260–F268: 3 o'clock (right of logo, ~(1280, 540))** — *"Save"* + ***"customers."***<br>**Icon: person silhouette → fills + check-stamp.** A line-style person icon appears (~48 px, head + shoulders outline). F0–F2: silhouette outline draws on. F2–F4: silhouette FILLS with soft purple from bottom up (like a meter filling) — the "AI is capturing the contact." F4–F6: small green check `#15803D` stamps in at the upper-right corner of the figure. F6–F8: continuous gentle glow on the check. **Visually says:** *"the customer is recognized and saved."*<br><br>**🗺 Feature 3 — F268–F276: 6 o'clock (below logo, ~(960, 860))** — *"Drive"* + ***"less."***<br>**Icon: 2 pins + route → optimization morph.** Two location pins appear (~16 px each, ~80 px apart). F0–F2: pins drop in. F2–F4: a wavy/zigzag dotted route draws between them (the "old way" — long, inefficient). F4–F6: route MORPHS — the wavy line snaps into a shorter, near-straight optimized path with subtle gradient `#3B82F6 → #6D28D9`. F6–F8: 2–3 small light particles flow along the optimized route. **Continuous:** light particles keep flowing along the route while the feature persists. **Visually says:** *"AI rerouted you to drive less."*<br><br>**🤝 Feature 4 — F276–F284: 9 o'clock (left of logo, ~(640, 540))** — *"Win more"* + ***"jobs."***<br>**Icon: speech bubble → paper airplane → checkmark stamp.** A line-style chat bubble appears (~36 px). F0–F2: bubble line-draws on. F2–F4: a paper-airplane icon flies up-and-right out of the bubble (motion blur trail). F4–F6: brief beat where airplane exits frame, then a large check `#15803D` stamps into the bubble's center (scale 0 → 1.15 → 1.0 spring). F6–F8: brief celebratory pulse on the check. **Continuous:** subtle glow halo on the check while feature persists. **Visually says:** *"AI sent the follow-up, the customer accepted, you won the job."*<br><br>**Icon style rules (apply to all 4):**<br>- Line-style (stroke-only, no solid fills except the per-feature transform moments described above — silhouette filling for #2, route gradient for #3, check stamp for #2 and #4)<br>- Stroke ~3 px, color white at 95% opacity<br>- Subtle purple glow `rgba(109,40,217,0.4)` blur 12 px UNDER the icon (matches the kinetic-typography purple underline aesthetic — ties the icon to the AI signal)<br>- Each icon's transformation animation lands within ~6 frames; the remaining ~2 frames of the per-feature 8f window are continuous "alive" motion<br>- Once placed, **all 4 icons keep their continuous motion in parallel** while the feature stays on screen — viewer sees a constellation of 4 micro-animations + the centered logo + sparkle. Lots of life, no dead pixels.<br><br>**By F284 the viewer sees:** centered Kiva logo + "Kiva." wordmark + 4 features (each = icon + text) arrayed at clock positions like satellites + the sparkle still orbiting + every icon continuously animating its own meaning. This is the *constellation moment* — every AI feature radiating its action from the brand at center. **Hold this constellation visible for a beat (F284 → vortex begins F286).**<br><br>**Audio (Phase 3 to refine):** 4 sparkle chimes at burst frames (F254, F262, F270, F278), each pitched +1 semitone higher. |
| 0:09.5–0:09.7 | 284–290 | **Constellation collapses — sparkles vortex to center.** All 4 feature texts dissolve simultaneously into purple particles (synchronized at F286), and those particles + the orbiting sparkle itself spiral inward toward the centered logo (~50 particles total — more than v1.24 because we're collapsing 4 persistent texts not just one). The orbit-trail dissolves. Camera zooms slightly forward 1.0× → 1.05×. Audio: filtered shimmer rising in pitch. |
| 0:09.7–0:10.1 | 290–302 | **Logo dissolves into iPhone.** The centered Kiva logo + "Kiva." wordmark fade opacity 1 → 0 over 8 frames (F290–F298) AS the 3D iPhone fades up in the same center position over 12 frames (F290–F302). Cross-fade transformation — the brand mark literally becomes the device. iPhone resting tilt rotateY -6°, rotateX +3° (per §3.7.2). Soft blue rim lighting + glass reflections establish. AI glow halo at idle (blue, 35%, 80 px blur). Audio: `iphone_morph_whirr.mp3` at F290–F300 (soft mechanical materialize). |
| 0:10.1–0:10.4 | 302–312 | **Dashboard appears.** Phone display lights up showing the Kiva dashboard (mic FAB visible, bottom nav, top header). At F306 small bottom-center text fades in: *"All your admin. One place."* (Inter_400Regular ~10 px, white at 70% opacity). Camera drift baseline running. **Hands off to Sequence 3** — picks up with the dashboard view and slow camera zoom toward mic button. Audio: soft "app loaded" chime. |
