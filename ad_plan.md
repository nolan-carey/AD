...
| 2 | **Swipe-up + centered brand + bubble-tagline pill + sparkle orbits logo + 4-feature TYPED+ACTIVE-STATE constellation + iPhone** | 0:06 – 0:11.9 | 180 – 357 | 5.9s | Vertical swipe-up clears → silence → **CENTERED** brand lockup (Kiva chevron + "Kiva." wordmark stay centered entire sequence) → bubble-textured glassmorphism tagline pill expands/inflates with bubble-pop SFX, holds with iridescent edge + sparkle particles + text-div breathing synced to highlight sweep, then pops out → AI sparkle emerges from logo → orbits w/ wobble dropping 4 features at organic positions (per v1.29) at **19f each (extended v1.32)**, text TYPES in char-by-char, icon TRANSFORMS into LIVE active-state UI w/ drifting simulated content (mic→red recording UI w/ "Quote for a standard toilet refit" drifting / person→customer card auto-filling Annie Yang etc / pins→route+drifting addresses / bubble→typing message+sent state) → constellation persists → particles vortex to center → logo cross-fades into iPhone → dashboard with "All your admin. One place." (v1.32) |
...
**Time:** 0:06.0 – 0:11.9 (frames 180–357 · 5.9s · 177 frames) · **v1.32: extended +1.5s for feature-flash pacing per user direction; per-feature window now 19f (was 8f); icon to RIGHT of text; text TYPES IN; icons transform into LIVE ACTIVE app states with drifting simulated content.**
