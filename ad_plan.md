...
| 2 | **Swipe-up + brand lockup + AI sparkle 4-feature flash + enter app** | 0:06 – 0:10.4 | 180 – 312 | 4.4s | Vertical swipe-up clears → silence → centered brand lockup (Kiva chevron + "Kiva." wordmark + tagline) → logo glides top-right (persists rest of ad) → AI sparkle director darts 4 quadrants → kinetic-typography feature flashes ("Speak quotes." / "Save customers." / "Drive less." / "Win more jobs.") → sparkle vortex → iPhone materializes → dashboard fades with "All your admin. One place." (v1.24) |
...
### Scene 2 — Swipe-up + brand lockup + AI sparkle feature flash + enter app
**Time:** 0:06.0 – 0:10.4 (frames 180–312 · 4.4s · 132 frames) · **v1.24: rewritten per user direction (2026-05-07) — replaces the v1.22 "iPhone + Kiva app opening" spec.**

> 📌 **v1.24 user direction (2026-05-07):** *"After 'Feeling overwhelmed?' is finished being typed. I want swipe-up transition that clears the screen and then Kiva logo appears centered the Kiva new line 'blue collar solutions for blue collar problems'. Then logo moves to top right, AI sparkle comes in and quickly lists 4 features before we enter the app."*
>
> User picked: **AI Sparkle as the Director** for the feature reveal + **declarative Jobber-voice copy** ("Speak quotes." / "Save customers." / "Drive less." / "Win more jobs.")

**Beat-by-beat (v1.24):**

| Range (s) | Frames | Action |
|---|---|---|
| 0:06.0–0:06.4 | 180–192 | **SWIPE-UP WIPE.** Vertical sweep from bottom of frame upward (1800 px/s). Notification cluster + "Feeling overwhelmed?" text + cursor all drag UP off the top of the frame with motion-blur trails. Background returns to clean navy → black cinematic gradient. **Audio:** `swoosh.mp3` at F180, played with slight pitch-up + mild reverse-tail flavor to feel "lifting" rather than horizontal. |
| 0:06.4–0:06.6 | 192–198 | **Silence beat.** Pure dark navy gradient, nothing on screen. **Audio: COMPLETE SILENCE** — the chaos is gone, the brand is about to arrive. |
| 0:06.6–0:07.2 | 198–216 | **Brand lockup fades up centered.** Three elements appear in sequence (FADE_RISE preset, each with subtle 4 px rise):<br>• **F198–F206:** Kiva chevron logo fades in centered (scale 0.9 → 1.0, spring damping 14)<br>• **F204–F212:** *"Kiva."* wordmark below the logo (gap 12 px) — Inter_600SemiBold ~36 px, white, **trailing period in `#3B82F6`** per §7 logo spec<br>• **F210–F216:** *"Blue collar solutions for blue collar problems"* tagline below the wordmark (gap 18 px) — Inter_400Regular ~16 px, white at 80% opacity<br>Subtle radial blue glow `rgba(59,130,246,0.3)`, blur 60 px, behind the lockup. Layered staggered entry — the logo lands first, wordmark a beat later, tagline a beat later. |
| 0:07.2–0:07.6 | 216–228 | **Brand lockup holds.** Static. Gentle glow pulse (opacity 0.8 → 1.0 → 0.8 over 12 frames). Viewer reads the tagline. **Audio:** soft `impact2.mp3` at 30% vol on F198 (logo land), then sustained ambient drone — no other hits. The hold IS the moment. |
| 0:07.6–0:08.2 | 228–246 | **Logo glides to top-right; wordmark + tagline fade out.**<br>• **Logo (chevron only):** translates from center → top-right corner (target position ~(1700, 100) on 1920×1080) AND scales 1.0 → 0.5 over 18 frames. Magnetic easing — accelerates slightly as it approaches the corner. Once parked top-right, **the chevron persists for the remainder of the ad** as a quiet brand presence (Norm: implement as a persistent overlay layer above the cinematic environment).<br>• **Wordmark + tagline:** fade out opacity 1 → 0 over 12 frames (F228–F240), no movement. They've done their job; only the mark stays.<br>**Audio:** subtle filtered whoosh, low and short, accompanying the glide. |
| 0:08.2–0:08.4 | 246–252 | **AI sparkle enters center stage.** Purple `<SparkleLoader>` (8-petal, ~60 px on the 1920×1080 frame, larger than the in-app version) materializes at center where the logo just left. Fades in opacity 0 → 1 over 6 frames + begins gentle clockwise rotation. Subtle purple glow `rgba(109,40,217,0.5)` blur 40 px around it. **Audio:** soft sparkle chime (`sparkle_match.mp3` if generated, else `notication1.mp3` pitched +5st at 30% vol). |
| 0:08.4–0:09.5 | 252–284 | **🎯 4-feature flash — AI Sparkle as the Director.** The sparkle is the protagonist; it darts to 4 quadrant positions; at each landing point, a feature TEXT BURSTS OUT from the sparkle. ~8 frames per feature.<br><br>**Per-feature pattern (8f each):**<br>F0–F2: sparkle DARTS to next position (motion blur trail, accelerates → decelerates)<br>F2: sparkle PULSES BRIGHT (scale 1.0 → 1.4 → 1.0) — burst of ~12 purple particles outward<br>F2–F5: text appears with kinetic typography (see below)<br>F5–F7: hold<br>F7–F8: text dissolves into purple particles + sparkle launches toward next position<br><br>**Kinetic typography rule:** each feature is split into TWO weights — the verb is small (~32 px Inter_400Regular at 80% opacity), the **outcome word is HUGE** (~64 px Inter_700Bold at 100% white) with a brief scale punch (1.0 → 1.08 → 1.0 over 3 frames). The period animates separately with a 1-frame delay. Soft purple glow underline beneath the bold word (~3 px tall, 0.5s fade in/out).<br><br>**Feature 1 — F252–F260: upper-left quadrant (~(480, 380))**<br>*"Speak"* (small) + ***"quotes."*** (HUGE, scale-punched). Purple underline beneath "quotes."<br><br>**Feature 2 — F260–F268: upper-right quadrant (~(1440, 380))**<br>*"Save"* (small) + ***"customers."*** (HUGE)<br><br>**Feature 3 — F268–F276: lower-left quadrant (~(480, 700))**<br>*"Drive"* (small) + ***"less."*** (HUGE — punchiest because shortest)<br><br>**Feature 4 — F276–F284: lower-right quadrant (~(1440, 700))**<br>*"Win more"* (small) + ***"jobs."*** (HUGE)<br><br>**Audio per feature:** soft sparkle chime at the burst frame (F254, F262, F270, F278) at 30% vol with each successive ding pitched +1 semitone (rising tension). NO per-character typing clicks per the v1.16 sound mix density rule. |
| 0:09.5–0:09.7 | 284–290 | **Sparkle vortex — particles converge.** All 4 dissolved feature-text particles + the sparkle itself swirl toward the center of the frame in a vortex (~30 particles, spiraling inward). Camera zooms slightly forward 1.0× → 1.05×. **Audio:** filtered shimmer rising in pitch over 6 frames. |
| 0:09.7–0:10.1 | 290–302 | **iPhone materializes from vortex.** The vortex compresses to a point at center, then a brilliant flare bursts and the 3D iPhone fades up from opacity 0 → 1. Resting tilt rotateY -6°, rotateX +3° (per §3.7.2). Soft blue rim lighting + glass reflections establish. AI glow halo at idle (blue, 35%, 80 px blur). **Audio:** `iphone_morph_whirr.mp3` (P1) at F290–F300 (soft mechanical materialize). |
| 0:10.1–0:10.4 | 302–312 | **Dashboard appears.** Phone display lights up showing the Kiva dashboard (mic FAB visible, bottom nav, top header). At F306 small bottom-center text fades in: *"All your admin. One place."* (Inter_400Regular ~10 px, white at 70% opacity). Camera drift baseline running. **Hands off to Scene 3** — Scene 3 picks up with the dashboard view and slow camera zoom toward mic button. **The chevron logo (top-right corner) is visible from F228 onward and remains throughout the rest of the ad.** |

**Scene 2 sound timeline (v1.24):**
- F180: `swoosh.mp3` at 100%, pitched +1st with reverse-tail flavor (the upward-lift feel)
- F186–F198: HARD SILENCE
- F198: soft `impact2.mp3` at 30% (logo land)
- F198 onward: ambient drone underbed begins (low warm pad, -22 dBFS)
- F232: subtle low filtered whoosh (logo glide to corner)
- F250: soft sparkle chime (sparkle entrance)
- F254, F262, F270, F278: 4× sparkle chimes (one per feature burst), each +1 semitone higher
- F284–F290: filtered shimmer rising in pitch (vortex)
- F290–F300: `iphone_morph_whirr.mp3` (iPhone materialize)
- F306: soft "app loaded" chime (`notication1.mp3` pitched +5st at 25% vol)
